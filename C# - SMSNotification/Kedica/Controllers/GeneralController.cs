...
namespace SMSNotification.Controllers
{
    public class GeneralController : Controller
    {
        ...
        public ActionResult GetSelect2Data()
        {
            ...
            if (query == "SELECT * FROM mGeneral WHERE [TypeID] = (SELECT ID FROM mTypes WHERE ID = 9) AND IsDeleted = 0 ")
            {
                var test = "";
            }
...

                {
                    ...
                    using (MySqlCommand cmdMySql = conn.CreateCommand())
                    {
                        ...
                        if (query == null || query == "")
                        {
                            ...
                            if (isDistict != "")
                            {
                                cmdMySql.CommandText = "SELECT DISTINCT(" + id + ")," + text + " FROM " + table + " WHERE IsDeleted=0 " + condition + " AND ( " + id + " like '%" + val + "%' OR " + text + " like '%" + val + "%')" + orderBy;
                            ...
                            {
                                cmdMySql.CommandText = "SELECT " + id + "," + text + " FROM " + table + " WHERE IsDeleted=0 " + condition + " AND ( " + id + " like '%" + val + "%' OR " + text + " like '%" + val + "%')" + orderBy;
                                using (MySqlDataReader sdr = cmdMySql.ExecuteReader())
                                {
                                    while (sdr.Read())
                                    {
